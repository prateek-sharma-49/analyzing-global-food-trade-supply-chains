...
# Step 2: Loading the dataset with a custom header location
...
# Extracting the Total-US Imports row
total_imports = dataset[dataset['Category'] == 'Total - US Imports']

# Extracting the years and corresponding import values
...
# Outer bar representing total US imports for each year
plt.bar(df['Year'], df['Total_Animals'] + df['Total_Plants'] + df['Total_Beverages'], color='skyblue', label='Total US Imports')

# Inner bars representing contributions from different categories
...
years = dataset_volume.columns[2:]  # Assuming years start from the third column
...
# Excluding categories that are not relevant to the analysis
...
# Group by food type and sum up the volumes from 1999 to 2022
...
# Defining the years for which you want to make predictions
future_years = [2023, 2024, 2025]

# Combining the years with the predicted volume values
...
# Calculate total volume for the year 2022
total_volume_2022 = sum(year_2022_data.values())

# Calculate the percentage of total volume for each country
...
# Outer bar representing total volume for each year
plt.bar(df.columns[1:], df.loc[df['Mode'] == 'Total'].values[0][1:], color='skyblue', label='Total Volume')

# Inner bars representing contributions from different modes of transportation with different colors
